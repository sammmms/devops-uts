"""
Comprehensive test suite for all backend API routes with temporary database isolation.
Run with: python test_all_routes.py
"""
...
if __name__ == "__main__":
    print("=" * 60)
    print("BACKEND API ROUTE TESTS (with Temp Database Isolation)")
    print("=" * 60)
    ...
    try:
        # Create empty JSON files for databases
        for filename in ["users.json", "categories.json", "todos.json"]:
            filepath = os.path.join(test_temp_dir, filename)
            with open(filepath, "w") as f:
                f.write("{}")
        
        # Reset singleton instances and point to temp directory
        UserDatabase._instance = None
        UserDatabase._db_path = os.path.join(test_temp_dir, "users.json")
        
        CategoryDatabase._instance = None
        CategoryDatabase._db_path = os.path.join(test_temp_dir, "categories.json")
        
        TodoDatabase._instance = None
        TodoDatabase._db_path = os.path.join(test_temp_dir, "todos.json")
